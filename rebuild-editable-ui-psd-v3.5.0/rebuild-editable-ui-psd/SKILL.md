...
This is the Codex Desktop + Windows Photoshop v3.5 branch. Use `references/codex-photoshop-v3.md` and the bundled automation scripts for assembly and layer export.
...
The source screenshot remains the authority for component identity, text, layout, and z-order. The scene appearance may drift because v3.5 defaults to whole-scene regeneration and direct replacement. Do not request separate scene-drift approval unless the user has explicitly required source-scene preservation.
...
1. **Preflight.** Run `scripts/check_environment.py`, record its rembg deployment result, then run `scripts/photoshop_bridge.py probe <work>/photoshop-probe --timeout 45`. Confirm permission before sending local artwork to a network service. Block native assembly when the Photoshop bridge fails.
...
Apply this selection inside the existing segmentation and matting actions; do not add, remove, reorder, or bypass any workflow phase or review gate.

- Before background removal, inspect the rembg deployment result from `scripts/check_environment.py`; rerun the check if the environment may have changed.
- When rembg is detected, use rembg as the preferred background-removal route, then apply all existing approved-mask, partial-alpha, silhouette, audit, and human-review rules unchanged.
- When rembg is not detected, explicitly notify the user that rembg was not detected, then continue with the existing segmentation and matting route unchanged.
