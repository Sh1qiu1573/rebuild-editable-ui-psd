...
| Detect rembg deployment | `scripts/check_environment.py` | Required before background removal |
...
Use the rembg result reported by `scripts/check_environment.py` before background removal. Treat rembg as deployed when either its Python module or CLI is detected.

- If rembg is detected, prefer it for background removal, then keep the existing segmentation, partial-alpha matting, mask-audit, and human-review requirements unchanged.
- If rembg is not detected, explicitly notify the user that rembg was not detected, then continue through the existing instance-segmentation and partial-alpha-matting route.

Do not install, deploy, or configure rembg as part of this skill. This selection changes only the background-removal tool, not the workflow or its gates.
...
Whole-scene drift is accepted by selecting v3.5 unless the user explicitly requires source-scene preservation. When preservation is required, stop and use a different workflow.
