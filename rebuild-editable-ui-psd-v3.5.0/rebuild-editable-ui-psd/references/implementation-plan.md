# Hybrid PSD v3.5 implementation plan
...
4. Record v3.5 scene policy: whole-scene regeneration and direct replacement are the default. Source-scene pixel preservation is not expected.
5. Run `scripts/check_environment.py` and record its rembg deployment result for the existing background-removal actions.
...
Scene-drift approval is implicit in selecting this v3.5 skill. Ask again only when the user separately requires original scene preservation.
...
Before either extraction route removes a background, use the recorded rembg deployment result; rerun the environment check if it may be stale. When rembg is detected, prefer it and preserve all existing mask, alpha, audit, and review requirements. When it is not detected, explicitly notify the user, then continue with the existing segmentation and matting route. This selection does not add or reorder a phase.
