# Hybrid PSD v3.2 implementation plan
...
4. Record v3.2 scene policy: whole-scene regeneration and direct replacement are the default. Source-scene pixel preservation is not expected.
...
Scene-drift approval is implicit in selecting this v3.2 skill. Ask again only when the user separately requires original scene preservation.
...
7. Run the Photoshop assembly job and save `final.psd` under a new name.
8. Export `preview.png` from the same approved composition or approved assembly job.
9. Confirm mechanically that required output files exist and are non-empty. Do not turn this into visual AI verification.
10. Deliver the package and `limitations.md`.
...
Completion criterion: required outputs exist, the approval record matches the assembled asset versions, and no post-approval AI review was performed.
