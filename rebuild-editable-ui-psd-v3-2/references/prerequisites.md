...
| Assemble native PSD | `photoshop_bridge.py` through Windows Photoshop | Required for `final.psd` |
...
V3.2 requires Codex Desktop on Windows and Adobe Photoshop registered as `Photoshop.Application`.

1. Run `scripts/photoshop_bridge.py probe <work>/photoshop-probe --timeout 45`.
2. Confirm native group, text, shape, and embedded smart-object support.
...
Whole-scene drift is accepted by selecting v3.2 unless the user explicitly requires source-scene preservation. When preservation is required, stop and use a different workflow.
