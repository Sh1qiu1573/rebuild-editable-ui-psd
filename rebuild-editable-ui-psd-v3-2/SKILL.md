---
name: rebuild-editable-ui-psd-v3-2
description: Rebuild a flattened app or game UI screenshot into a hybrid editable Photoshop PSD on Windows using whole-scene regeneration, whole-component redraw for occluded raster artwork, two blocking human-review gates, semantic one-object-per-layer extraction, measured text and shape reconstruction, and deterministic Photoshop assembly. Use when the user asks to split, reverse-engineer, reconstruct, or convert a PNG/JPG interface screenshot into an editable layered PSD and accepts a regenerated scene plus human approval instead of post-assembly AI verification.
---

# Rebuild Editable UI PSD v3.2
...
This is the Codex Desktop + Windows Photoshop v3.2 branch. Use `references/codex-photoshop-v3.md` and the bundled automation scripts for assembly.
...
The source screenshot remains the authority for component identity, text, layout, and z-order. The scene appearance may drift because v3.2 defaults to whole-scene regeneration and direct replacement. Do not request separate scene-drift approval unless the user has explicitly required source-scene preservation.
...
8. **Assemble and hand off.** Build the PSD from the approved manifest and approved assets through the Photoshop bridge. Export `preview.png` from the same approved composition, save under a new name, and deliver the PSD, review records, manifest, assets, masks, generation logs, and limitations report. Perform only mechanical output checks needed to confirm that files were written; do not add a post-assembly AI review phase.
