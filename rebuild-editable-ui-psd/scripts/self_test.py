...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="rebuild-ui-skill-") as temp:
        ...
        title_art_job = json.loads(json.dumps(export_job))
        title_art_job["layers"].append({
            "id": "event_title_art", "name": "Img_EventTitle", "kind": "raster-object", "z": 150,
            "source_asset": str(source_path), "bounds": [40, 90, 280, 160],
        })
        validated_title_art = validate_job(title_art_job, work)
        title_art = next(item for item in validated_title_art["layers"] if item["id"] == "event_title_art")
        if title_art.get("text_fallback") is True or not title_art["name"].startswith("Img_"):
            raise RuntimeError("Artistic title artwork was incorrectly treated as a text fallback")
