...
def shape_job(root: Path, name: str, radius: int) -> dict:
    return {
        ...
        "output": {
            ...
            "layer_png_dir": str(root / name / "png"),
    ...
    return {
        ...
        "layers": [
            ...
            },
            {
                ...
                "bounds": [45, 32, 255, 84], "baseline_y": 82, "text": "Codex v3.6.2", "font": font,
...
def assert_probe(work: Path, timeout: float) -> None:
    ...
    expected = {"Group": 3, "ShapeLayer": 3, "TypeLayer": 1, "SmartObjectLayer": 2}
    ...
    root_shape = next(layer for layer in structure["layers"] if layer["name"] == "Bg_ProbeRoot")
    if root_shape["depth"] != 0:
        raise RuntimeError(f"Unparented native shape was nested after reopen: depth={root_shape['depth']}")
    ...
    expected_names = {"Bg_ProbeScene", "Bg_ProbeCanvas", "Bg_ProbeRoot", "Btn_Probe", "Bg_ProbeButton", "@ProbeText", "Icon_Probe"}
    ...
    if {"00_REFERENCE", "source_reference"} & exported_names:
        raise RuntimeError("Reference layers were exported as PNG files")
    for item in exported:
        layer_png = Image.open(item["path"]).convert("RGBA")
        if layer_png.size != (360, 220):
            raise RuntimeError(f"Layer PNG canvas changed for {item['name']}: {layer_png.size}")
        alpha = np.asarray(layer_png.getchannel("A"))
        if item["name"] not in {"Bg_ProbeScene", "Bg_ProbeCanvas"} and not np.any(alpha == 0):
            raise RuntimeError(f"Layer PNG lost its transparent background: {item['name']}")
