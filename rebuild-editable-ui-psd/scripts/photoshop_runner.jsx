...
(function () {
    ...
    function moveIntoParent(layer, item) {
        ...
        } else if (!item.parent && layer.parent && layer.parent.typename === "LayerSet") {
            var document = app.activeDocument;
            var rootAnchor = document.layers.length ? document.layers[0] : null;
            if (rootAnchor && rootAnchor !== layer) {
                layer.move(rootAnchor, ElementPlacement.PLACEBEFORE);
            }
        }
    }
    ...
    function jobItemById(idValue) {
        var index;
        for (index = 0; index < JOB.layers.length; index += 1) {
            if (JOB.layers[index].id === idValue) { return JOB.layers[index]; }
        }
        throw new Error("Cannot resolve layer job item: " + idValue);
    }
    function findNamedChild(container, item) {
        var expectedName = item.name || item.id;
        var index;
        for (index = 0; index < container.layers.length; index += 1) {
            var candidate = container.layers[index];
            var typeMatches = item.kind === "group"
                ? candidate.typename === "LayerSet"
                : candidate.typename !== "LayerSet";
            if (typeMatches && candidate.name === expectedName) { return candidate; }
        }
        throw new Error("Layer is missing after reopen: " + expectedName);
    }
    function findReopenedLayer(document, itemId) {
        var chain = [];
        var item = jobItemById(itemId);
        while (item) {
            chain.unshift(item);
            item = item.parent ? jobItemById(item.parent) : null;
        }
        var container = document;
        var found = null;
        var index;
        for (index = 0; index < chain.length; index += 1) {
            found = findNamedChild(container, chain[index]);
            container = found;
        }
        return found;
    }
    function createLayerExportDocument(sourceDocument, nameValue) {
        var profile = "sRGB IEC61966-2.1";
        try { profile = sourceDocument.colorProfileName || profile; } catch (ignoredProfile) {}
    ...
    function exportLayerPngs(sourceDocument, pngOptions) {
        var exported = [];
        var index;
        for (index = 0; index < JOB.layer_png_exports.length; index += 1) {
            var exportItem = JOB.layer_png_exports[index];
            app.activeDocument = sourceDocument;
            var sourceLayer = findReopenedLayer(sourceDocument, exportItem.id);
            var exportDocument = createLayerExportDocument(sourceDocument, "codex-layer-" + exportItem.id);
            var bootstrap = exportDocument.artLayers.length === 1 && exportDocument.layerSets.length === 0
                ? exportDocument.artLayers[0]
                : null;
            app.activeDocument = sourceDocument;
            var duplicated = sourceLayer.duplicate(exportDocument, ElementPlacement.PLACEATBEGINNING);
            app.activeDocument = exportDocument;
            duplicated.visible = true;
            if (bootstrap) { bootstrap.remove(); }
            var outputFile = ensureFolder(JOB.output.layer_png_dir + "/" + exportItem.filename);
    ...
    function makeGroup(item) {
        ...
        if (item.opacity !== undefined) { group.opacity = numberValue(item.opacity, 100); }
    ...
    function build() {
        ...
        var layerPngs = exportLayerPngs(reopened, pngOptions);
        ...
        var report = {
            ...
            layer_png_dir: new Folder(JOB.output.layer_png_dir).fsName,
            layer_pngs: layerPngs,
