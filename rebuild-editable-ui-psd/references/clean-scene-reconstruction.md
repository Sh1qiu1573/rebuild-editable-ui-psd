...
V3.6.2 uses `full-scene-regeneration` by default. Invoke the Codex `$imagegen` skill in its default built-in mode, generate one complete UI-free scene, and use it as the entire scene smart object.
...
Selecting this skill accepts scene drift unless the user separately states that visible scene pixels must remain unchanged. Follow `$imagegen` prompt shaping, reference-image, built-in save-path, inspection, and iteration rules. Move or copy the selected project asset into the task workspace. Do not request an API key or call a CLI/external image API by default. If source preservation becomes a requirement, stop and use a source-preserving workflow instead of silently reverting to masked patching.
...
3. Invoke `$imagegen` in built-in mode to generate complete full-frame candidates.
...
- `generation_route: codex-imagegen-built-in`, `imagegen_skill: imagegen`, and `credential_route: signed-in-account`.
- Built-in generated source path, final workspace path, and any explicitly approved fallback route.
...
- **The user now requires original scene preservation:** stop and switch to a source-preserving workflow; do not introduce masked splicing into v3.6.2.
- **Built-in `$imagegen` is unavailable:** report that its CLI fallback requires explicit user choice and may require `OPENAI_API_KEY`; do not switch silently.
