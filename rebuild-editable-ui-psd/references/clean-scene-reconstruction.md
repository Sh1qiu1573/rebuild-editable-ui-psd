...
V3.6 uses `full-scene-regeneration` by default. Generate one complete UI-free scene with Codex Desktop's built-in GPT Image 2 and use it as the entire scene smart object.
...
Selecting this skill accepts scene drift unless the user separately states that visible scene pixels must remain unchanged. Use the current signed-in Codex Desktop account allowance and do not call an external image API by default. If source preservation becomes a requirement, stop and use a source-preserving workflow instead of silently reverting to masked patching.
...
- `generation_route: codex-desktop-gpt-image-2` and `credential_route: signed-in-account`.
...
- **The user now requires original scene preservation:** stop and switch to a source-preserving workflow; do not introduce masked splicing into v3.6.
