...
Reconstruct both front and back objects as complete/amodal objects before stacking. Never cut the back shape to its visible fragment. For regular or custom-vector frames, extend hidden tangent and parallel edges geometrically; use Codex Desktop's built-in GPT Image 2 only for hidden irregular raster artwork, including artistic title artwork when incomplete. Preserve each button's own outer stroke, inner shadow/glow, bevel, texture, and drop shadow before evaluating the combined render.
