...
Name native ordinary text layers and approved ordinary-text raster fallbacks with `@`. Mark an ordinary-text raster fallback with `text_fallback: true` in the Photoshop job so validation can distinguish it from an incorrectly named image.

Name decorative or artistic headline lettering with `Img_` or `Image_`, for example `Img_EventTitle`. It is raster artwork, not an `@` text layer and not a `text_fallback`, even when it contains readable words.
