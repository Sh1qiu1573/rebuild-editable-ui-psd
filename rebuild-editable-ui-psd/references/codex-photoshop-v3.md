# Codex Desktop + Photoshop v3.6.2 execution contract
...
1. Run `python scripts/install_rembg.py --ensure`, then `python scripts/check_environment.py --json`; require CPU fallback and record whether verified GPU or CPU is selected.
2. Run `python scripts/photoshop_bridge.py probe <work>/photoshop-probe --timeout 45`.
3. Complete human review 1.
4. Invoke `$imagegen` in default built-in mode for the whole scene and any required whole-component redraws; produce the approved manifest, raster assets, artistic title artwork, ordinary editable text, shapes, and overlap order.
...
Before compilation, the bridge also enforces the semantic naming and button structure contract in `psd-layer-structure.md`, including top-level placement for every button group. `00_REFERENCE` and descendants are exempt. Use `text_fallback: true` only for an approved ordinary-text raster/smart-object fallback whose layer name still starts with `@`. Artistic title lettering is `Img_`/`Image_` raster artwork and must not use `text_fallback`.
...
Ordinary-text font selection and approximate styling occur before review 2. Broad similarity is sufficient unless the user explicitly requests close typography matching. The review composite must show the exact text/shape/title-art representation intended for the PSD.
