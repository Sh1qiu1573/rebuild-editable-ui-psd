...
- Generation route `codex-imagegen-built-in` and `imagegen_skill: imagegen` for every redraw; never record external API credentials.
- Alpha bounds, crop origin, padding, and final placement.

## Bundled rembg route

Before removing a background, run `scripts/install_rembg.py --ensure` during installation or preflight, verify the GPU-first and isolated CPU runtimes with `scripts/check_environment.py`, and invoke `scripts/rembg_cutout.py --backend auto`.

- Use rembg by default for every source extraction and every generated whole-component re-segmentation. Use GPU only after CUDA provider and actual inference validation; retry CPU automatically if a GPU object invocation fails.
...
- Record `backend_requested`, `backend_used`, `fallback_used`, and per-attempt errors for each object.
- If the CPU fallback runtime is missing, rerun the installer. If CPU installation or inference fails again, block extraction and report the exact error instead of using an external background-removal API.
...
2. Invoke the Codex `$imagegen` skill once for that object and use its default built-in mode. Follow its reference-image, prompt, inspection, iteration, and save-path rules; move or copy the selected output into the task workspace. Do not request or use an external API key by default.
...
- **Built-in `$imagegen` is unavailable:** explain the `$imagegen` CLI fallback and request the explicit approval it requires before using any API/CLI route or credentials.
