# V3.6.2 representation and human quality gates

## 1. Acceptance authority

V3.6.2 uses two blocking human-review gates:
...
1. Native Photoshop text for ordinary UI copy when readable and broadly similar, or a native shape for credible editable construction.
2. Custom vector path for crisp nonstandard contours.
3. Whole-component raster smart object for artistic headline lettering, irregular, textured, distressed, damaged, or occluded artwork.
...
Use exact wording and separate spatially distant ordinary text units. Approximate font category, weight, outline, tracking, size, rotation, and color; broad similarity is sufficient and exhaustive font search is not required.

Create live TypeLayers for ordinary readable UI copy. Treat decorative or artistic headline lettering as `Img_`/`Image_` raster artwork rather than editable text or a text fallback. When ordinary typography is unavailable, use a documented raster fallback or human-approved substitution. Do not rely on an AI post-assembly regression score.
...
- Fully visible objects may be extracted from source with verified GPU-first rembg, automatic CPU retry, and approved masks.
...
- Re-segment every whole-component redraw with bundled rembg, then refine and re-matte it.
...
- Every expected non-reference layer PNG exists, is non-empty, and is listed in the Photoshop report.
