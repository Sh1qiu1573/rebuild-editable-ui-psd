# Hybrid PSD v3.6 implementation plan
...
2. Record paths, hashes, dimensions, color mode, profile, output path, font availability, reference-asset reuse permission, whether built-in GPT Image 2 is prohibited, and permission for any third-party network route.
3. Record the source screenshot as authority for identity, text, geometry, layout, and z-order.
4. Record v3.6 scene policy: whole-scene regeneration and direct replacement are the default. Source-scene pixel preservation is not expected.
5. Run `scripts/install_rembg.py --ensure`, then `scripts/check_environment.py`, and record the managed rembg runtime. Use Codex Desktop's signed-in GPT Image 2 allowance by default; never request external API credentials unless the user explicitly chooses a third-party provider.
...
Scene-drift approval is implicit in selecting this v3.6 skill. Ask again only when the user separately requires original scene preservation.
...
Before either extraction route removes a background, ensure the managed rembg runtime exists and invoke `scripts/rembg_cutout.py`. Preserve all existing mask, alpha, audit, and review requirements. If the runtime is missing, rerun `scripts/install_rembg.py --ensure`; if installation or inference still fails, block extraction rather than silently using another service. This requirement does not add or reorder a review phase.
...
2. Generate a complete replacement of the entire component in one dedicated built-in GPT Image 2 job using the signed-in Codex Desktop account allowance.
