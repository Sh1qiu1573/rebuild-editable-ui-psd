# Hybrid PSD v3.6.2 implementation plan
...
- Every ordinary label, number, and paragraph is editable text with exact wording and broadly similar styling, or a documented fallback.
- Every decorative or artistic headline is treated as separately selectable raster artwork rather than a live TypeLayer.
...
2. Record paths, hashes, dimensions, color mode, profile, output path, font availability, reference-asset reuse permission, whether the `$imagegen` built-in route is prohibited, and permission for any CLI/API or third-party network route.
3. Record the source screenshot as authority for identity, text, geometry, layout, and z-order.
4. Record v3.6.2 scene policy: whole-scene regeneration and direct replacement are the default. Source-scene pixel preservation is not expected.
5. Run `scripts/install_rembg.py --ensure`, then `scripts/check_environment.py`. Require the isolated CPU fallback and record NVIDIA detection, GPU/CPU runtime paths, provider lists, inference probes, and selected backend. Load `$imagegen` and use its default built-in mode for generation; never request external API credentials unless the user explicitly chooses or confirms the fallback under `$imagegen` rules.
...
2. Invoke `$imagegen` in its default built-in mode and generate a complete replacement scene at the target aspect ratio.
...
Scene-drift approval is implicit in selecting this v3.6.2 skill. Ask again only when the user separately requires original scene preservation.
...
Before either extraction route removes a background, ensure the GPU-first rembg installation and isolated CPU fallback exist and invoke `scripts/rembg_cutout.py --backend auto`. Preserve all existing mask, alpha, audit, and review requirements. Record the requested backend, actual backend, and fallback status for every object. If GPU fails, allow the built-in CPU retry. If CPU installation or inference fails, block extraction rather than silently using another service. This requirement does not add or reorder a review phase.
...
2. Invoke `$imagegen` once in its default built-in mode to generate a complete replacement of the entire component, then move or copy the selected result into the task workspace.
...
1. Classify decorative or artistic headline lettering as `raster-object` artwork and name it `Img_`/`Image_`; do not route it through live-text matching or `text_fallback`.
2. Split ordinary spatially distant text clusters into separate text units.
3. Transcribe exact wording and punctuation.
4. Match position, scale, alignment, rotation, fill color family, and visual hierarchy approximately. Use available licensed fonts without exhaustive similarity search.
5. Create live TypeLayers for ordinary text when readable; otherwise use a documented fallback plus optional hidden transcription.
...
- Incorrect wording, text grouping, ordinary-text readability/hierarchy, raster title artwork, or button construction.
...
- **GPU provider or inference probe fails:** use the verified CPU runtime and record the reason; do not block extraction when CPU succeeds.
- **GPU invocation fails during an object:** accept the automatic CPU retry only when it succeeds and record the fallback event.
- **CPU fallback fails:** block extraction and report both GPU and CPU errors.
- **Built-in `$imagegen` is unavailable:** report the explicit CLI/API fallback offered by `$imagegen`; do not use it without the user's required confirmation.
