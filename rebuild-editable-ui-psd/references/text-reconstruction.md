...
Use `raster-object` for a decorative or artistic headline when the lettering is logo-like, illustrated, highly distorted, textured, extruded, integrated with ornaments, or otherwise functions as a graphic. Name it `Img_<SemanticName>` or `Image_<SemanticName>`, not `@...`, and do not set `text_fallback: true`. Preserve it as extracted artwork when complete or invoke `$imagegen` in its default built-in mode to redraw the entire title artwork when damaged or occluded.
